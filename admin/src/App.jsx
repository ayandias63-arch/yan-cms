    ...
    <main className="login-shell">
      <section className="login-visual">
        ...
        <div className="visual-copy">
          <span className="eyebrow">YAN CMS / ADMIN</span>
          <h1>Seu conteúdo, em ordem.</h1>
          <p>
            Uma forma mais tranquila de gerenciar cada publicação e cada
            cliente.
          </p>
        </div>
        <div className="visual-note">
          Criado para equipes que publicam com intenção.
        </div>
      </section>
      <section className="login-panel">
        ...
        <div className="login-heading">
          ...
          <p>Acesse o espaço de administração do seu conteúdo.</p>
        </div>
        ...
        <div className="login-footer">
          <span className="status-dot" /> Sistema operacional
        </div>
      </section>
    </main>
  );
...
function Dashboard({ session, activeView, onNavigate, onLogout }) {
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        if (clientsResponse.status === 401 || articlesResponse.status === 401) {
          onLogout();
          return;
        }
        const clients = clientsResponse.ok ? await clientsResponse.json() : [];
        const articles = articlesResponse.ok
        ...
        setClients([]);
        setStats({ clients: 0, articles: 0, published: 0 });
      } finally {
        setLoading(false);
      }
    };
  ...
  if (session.user.role === "client")
    return <ClientDashboard session={session} onLogout={onLogout} />;
  return (
        ...
        <div className="sidebar-bottom">
          <div className="help-card">
            ...
            <div>
              ...
              <small>Consulte a documentação</small>
            </div>
          </div>
      ...
      <main className="dashboard-main">
        <header className="topbar">
          <div className="mobile-title">
            <span className="brand-mark">Y</span> Yan CMS
          </div>
        ...
        <div className="content-wrap">
          ...
          {activeView === "articles" && (
            <EmptyView
              title="Artigos"
              description="Administre e publique o conteúdo dos seus clientes aqui."
          ...
          {activeView === "clients" && (
            <ClientList
              clients={clients}
              canCreate={canManageAllClients}
              canDelete={canManageAllClients}
              onCreate={() => {
              ...
              onDelete={async (client) => {
                if (
                  !window.confirm(`Deseja excluir o cliente "${client.name}"?`)
                )
                  return;
                ...
                if (!response.ok) {
                  const data = await response.json();
                  window.alert(
                    data.message || "Não foi possível excluir o cliente",
                  );
                  return;
                }
                setClients((current) =>
                ...
                setStats((current) => ({
                  ...current,
                  clients: Math.max(0, current.clients - 1),
                }));
              }}
            />
          )}
...
function ClientDashboard({ session, onLogout }) {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        const headers = { Authorization: `Bearer ${session.token}` };
        const [clientResponse, articlesResponse, contentResponse] =
          await Promise.all([
            fetch(`${API_URL}/api/clients`, { headers }),
            fetch(`${API_URL}/api/articles`, { headers }),
            fetch(`${API_URL}/api/site-content`, { headers }),
          ]);
        if (
        ...
        const nextArticles = articlesResponse.ok
          ? await articlesResponse.json()
          : [];
        const nextContent = contentResponse.ok
          ? await contentResponse.json()
          : null;
        setClient(clients[0] || null);
        setArticles(nextArticles);
        setSiteContent(nextContent);
        if (!clientResponse.ok || !articlesResponse.ok || !contentResponse.ok)
          setError("Não foi possível carregar todos os dados do site");
      } catch {
  ...
  const removeArticle = async (article) => {
    ...
    const response = await fetch(`${API_URL}/api/articles/${article._id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${session.token}` },
    });
  ...
  const setArticleStatus = async (article, status) => {
    ...
    if (response.status === 401) return onLogout();
    if (!response.ok)
      return setError("Não foi possível atualizar o status do artigo");
  ...
  const saveArticle = (savedArticle) => {
    setArticles((current) =>
      articleToEdit
        ? current.map((item) =>
            item._id === savedArticle._id ? savedArticle : item,
          )
  ...
  const navItems = [
    { id: "overview", label: "Visão geral", icon: "⌂" },
    { id: "articles", label: "Meus artigos", icon: "▤" },
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="workspace-switcher">
          ...
          <span>
            <b>{client?.name || "Meu espaço"}</b>
            <small>Painel do cliente</small>
          </span>
        </div>
        <nav className="main-nav" aria-label="Navegação do cliente">
        ...
        <div className="content-wrap">
          ...
          {activeView === "overview" && (
            <ClientOverview
              ...
              loading={loading}
              onArticles={() => setActiveView("articles")}
              onSite={() => setActiveView("site")}
            />
          )}
          {activeView === "articles" && (
            <ArticleList
    ...
    <>
      <section className="welcome-row">
        <div>
          ...
          <h1>
            Olá, {client?.name || "cliente"} <span className="wave">✦</span>
          </h1>
          <p>Gerencie o conteúdo e as informações do seu próprio site.</p>
        </div>
        <button
      ...
      <section className="stats-grid">
        ...
        <StatCard
          ...
          detail={client?.domain || "Sem domínio"}
          icon="◉"
          tone="orange"
          loading={loading}
        />
      ...
      <section className="dashboard-grid">
        <div className="panel recent-panel">
          <div className="panel-heading">
            <div>
              <span className="eyebrow">SEU SITE</span>
              <h2>{client?.name || "Meu site"}</h2>
            </div>
            <button className="text-button" type="button" onClick={onSite}>
              Editar dados <span>→</span>
            </button>
          </div>
          <div className="client-site-summary">
            <strong>{client?.domain || "Domínio não informado"}</strong>
            <span>/{client?.slug || "sem-slug"}</span>
            <p>Atualize os dados do seu site na seção Meu site.</p>
          </div>
        </div>
        <div className="panel quick-panel">
          ...
          <button className="quick-action" type="button" onClick={onArticles}>
            <span className="quick-icon article-icon">▤</span>
            <span>
              <b>Meus artigos</b>
              <small>Crie e publique conteúdo</small>
            </span>
            <span>→</span>
          </button>
          <button className="quick-action" type="button" onClick={onSite}>
            <span className="quick-icon client-icon">◉</span>
            <span>
              <b>Dados do site</b>
              <small>Edite suas informações</small>
            </span>
            <span>→</span>
          </button>
        </div>
...
function ClientSite({ client, content, onEdit }) {
  return (
    <>
      <section className="empty-page">
        <span className="eyebrow">MEU SITE</span>
        <div className="page-heading">
          <div>
            <h1>{content?.mainTitle || client?.name || "Meu site"}</h1>
            <p>
              {content?.description ||
                "Edite o conteúdo público do seu próprio site."}
            </p>
          </div>
          <button
            className="primary-button compact"
        ...
        <div className="site-details-grid">
          ...
          <div>
            <span>Contato</span>
            <strong>{content?.contact?.email || "Não informado"}</strong>
            <small>{content?.contact?.phone || ""}</small>
          </div>
          <div>
            <span>WhatsApp</span>
            <strong>{content?.whatsapp || "Não informado"}</strong>
          </div>
          <div>
            <span>Serviços</span>
            <strong>
          ...
          <div>
            <span>Domínio do Client</span>
            <strong>{client?.domain || "Não informado"}</strong>
          </div>
        </div>
      </section>
    </>
...
function ArticleList({ articles, onCreate, onEdit, onDelete, onStatus }) {
  return (
    <section className="empty-page">
      ...
      <div className="page-heading">
        <div>
          <h1>Meus artigos</h1>
          <p>Gerencie apenas o conteúdo do seu site.</p>
        </div>
        <button
          className="primary-button compact"
          type="button"
      ...
      <div className="article-list">
        {articles.length === 0 ? (
          <div className="empty-page-box">
            <span className="large-empty-icon">✧</span>
            <h2>Ainda não há artigos</h2>
            <p>Crie o primeiro artigo do seu site.</p>
          </div>
        ) : (
          articles.map((article) => (
            <article className="article-row" key={article._id}>
              <div className="article-row-main">
                ...
                <small>
                  /{article.slug} · {article.category || "Sem categoria"}
                </small>
              </div>
...
function LegacyArticleForm({ session, article, onClose, onSaved, onLogout }) {
  ...
  const submit = async (event) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/articles${editing ? `/${article._id}` : ""}`,
        {
      ...
      if (response.status === 401) return onLogout();
      if (!response.ok)
        throw new Error(data.message || "Não foi possível salvar o artigo");
      onSaved(data);
    } catch (requestError) {
      setError(requestError.message || "Não foi possível conectar ao servidor");
    } finally {
  ...
  return (
    ...
    >
      <section
        className="client-modal article-modal"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="modal-heading">
          <div>
            ...
            <h2 id="article-form-title">
              {editing ? "Editar artigo" : "Criar artigo"}
            </h2>
            <p>Este conteúdo ficará vinculado ao seu site.</p>
          </div>
          <button
        ...
        <form className="client-form article-form" onSubmit={submit}>
...
function SiteForm({
  ...
  const initial = {
    ...
    sectionTexts: (content?.sectionTexts || []).join("\n"),
  ...
  const submit = async (event) => {
    ...
    const payload = {
      ...
      sectionTexts: form.sectionTexts
        .split("\n")
        .map((item) => item.trim())
        .filter(Boolean),
      contact: {
    ...
    try {
      ...
      if (response.status === 401) return onLogout();
      if (!response.ok)
        throw new Error(data.message || "Não foi possível salvar o conteúdo");
      onSaved(data);
    } catch (requestError) {
      setError(requestError.message || "Não foi possível conectar ao servidor");
    } finally {
  ...
  return (
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="site-content-form-title"
      >
        <div className="modal-heading">
          <div>
            <span className="eyebrow">MEU SITE</span>
            <h2 id="site-content-form-title">Editar conteúdo do site</h2>
            <p>Este conteúdo pertence apenas ao seu Client.</p>
          </div>
          <button
            className="modal-close"
            type="button"
        ...
        <form className="client-form article-form" onSubmit={submit}>
          ...
          <label htmlFor="content-sections">
            Textos das seções
            <textarea
              id="content-sections"
              name="sectionTexts"
              value={form.sectionTexts}
              onChange={update}
              rows="4"
              placeholder="Um texto por linha"
            />
          </label>
          ...
          <label htmlFor="content-whatsapp">
            WhatsApp
            <input
              id="content-whatsapp"
              name="whatsapp"
          ...
          <label htmlFor="content-instagram">
            Instagram
            <input
          ...
          <label htmlFor="content-facebook">
            Facebook
            <input
              id="content-facebook"
              name="facebook"
...
function ImageUpload({
  ...
  const upload = async (event) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/site-images/${field}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${session.token}` },
        body,
      });
  ...
  return (
    <div className="image-upload">
      ...
      {value && (
        <>
          <img src={mediaUrl(value)} alt={`${label} atual`} />
          <button
            className="client-action delete"
            type="button"
            onClick={remove}
...
function Overview({ stats, loading }) {
  return (
    <>
      <section className="welcome-row">
        <div>
          <span className="eyebrow">SÁBADO, 05 DE SETEMBRO DE 2026</span>
          <h1>
            Bom dia, Admin <span className="wave">✦</span>
          </h1>
          <p>Aqui está o que está acontecendo no seu CMS.</p>
        </div>
        <button className="primary-button compact">+ Criar artigo</button>
      </section>
      <section className="stats-grid">
        <StatCard
          label="Clientes ativos"
          value={stats.clients}
          detail="Espaços administrados"
          icon="◉"
          tone="orange"
          loading={loading}
        />
      ...
      <section className="dashboard-grid">
        <div className="panel recent-panel">
          ...
          <div className="empty-state">
            ...
            <b>Sua atividade aparecerá aqui</b>
            <p>
              Quando você criar artigos ou clientes, verá as últimas
              movimentações neste espaço.
            </p>
          </div>
        </div>
        <div className="panel quick-panel">
          ...
          <button className="quick-action">
            ...
            <span>
              <b>Novo artigo</b>
              <small>Compartilhe uma nova história</small>
            </span>
            <span>→</span>
          </button>
          <button className="quick-action">
            ...
            <span>
              <b>Novo cliente</b>
              <small>Adicione um espaço de trabalho</small>
            </span>
            <span>→</span>
          </button>
        </div>
...
function EmptyView({ title, description, count, onCreate }) {
  return (
    <section className="empty-page">
      ...
      <div className="page-heading">
        ...
        <button
          ...
          onClick={onCreate}
        >
          + Criar {title === "Artigos" ? "artigo" : "cliente"}
        </button>
      </div>
      <div className="empty-page-box">
        <span className="large-empty-icon">✧</span>
        <h2>
          {count
            ? `${count} registros disponíveis`
            : `Ainda não há ${title.toLowerCase()}`}
        </h2>
        <p>
          Esta visualização está pronta para conectar seus fluxos de gestão.
        </p>
      </div>
    </section>
  );
  ...
  return (
    <section className="empty-page">
      ...
      <div className="page-heading">
        <div>
          <h1>Clientes</h1>
          <p>Seus espaços de conteúdo aparecerão nesta visualização.</p>
        </div>
        {canCreate && (
          <button
            className="primary-button compact"
...
function ClientForm({
  ...
  const [form, setForm] = useState(
    client
      ...
      : { name: "", slug: "", domain: "", logo: "", status: "active" },
  );
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  ...
  const submit = async (event) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/clients${isEditing ? `/${client._id}` : ""}`,
        {
          ...
          <div>
            ...
            <p>
              {isEditing
                ...
                : "Adicione um novo espaço de conteúdo ao Yan CMS."}
            </p>
          </div>
          <button
            className="modal-close"
